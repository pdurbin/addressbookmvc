...
App.controller('addCtrl', ['$scope', '$http', '$location', '$routeParams', function ($scope, $http, $location, $routeParams) {
  ...
  if ($routeParams.id !== undefined) {
    $http.get('/addresses/search?id=' + $routeParams.id).success(function (data) {
...
App.controller('listCtrl', ['$scope', '$http', function ($scope, $http) {
  ...
  $http.get('/addresses/list').success(function (data) {
    $scope.contacts = data;
  });
  ...
  $scope.deleteContact = function (contactModel) {
    if (confirm("Are you sure?")) {
      $http.get('/addresses/delete?id=' + contactModel.id).success(function (data) {
  ...
  $scope.$on('searchBroadcast', function (event, args) {
    ...
    $http.get('/addresses/list?full_name=' + $scope.filter_full_name + '&email=' + $scope.filter_email).success(function (data) {
      $scope.contacts = data;
    });
  });
}]);
